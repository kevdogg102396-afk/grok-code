...
registerTool({
  name: 'subagent',
  description: 'Dispatch a task to a sub-agent running on a different Grok model. Use "fast" for quick tasks, "reason" for complex analysis, "latest" for Grok 4.3, or "standard" for general coding. The sub-agent has access to all the same tools.',
  parameters: {
    type: 'object',
    properties: {
      ...
      model: { type: 'string', description: 'Model: "fast" (Grok Code Fast), "standard" (Grok 4.20), "reason" (Grok 4.20 Reason), "latest" (Grok 4.3)' },
    },
    required: ['action'],
  },
  async execute(args) {
